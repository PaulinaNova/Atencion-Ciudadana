...
import { COLUMNS } from "./ColumnsEventos";
import "./Catalogo.css";
...
export const TableEventos = () => {
  ...
  const getData = async () => {
    const resp = await axios.get("/api/evento/");
    setEventos(resp.data);
  };

  function updtPut() {
    axios
      .put("/api/evento/updtEvento/" + campos._id, {
        nombre: values.evento,
      })
      .then((response) => {
  ...
  function dltEliminar() {
    axios
      .delete("/api/evento/dltEvento/" + campos._id)
      .then((response) => {
        setValues(response.data);
      });
    NotificationManager.success(
      ...
      >
        <div className="groupInput">
          ...
          <input
            ...
            onBlur={handleBlur}
            className={
              errors.evento && touched.evento ? "input-error" : ""
            }
          />
  ...
  const bodyDlt = (
    <div className={styles.modal}>
      <h3>¿Está seguro que desea eliminar este evento?</h3>
